...
def test_list_release_id_candidates() -> None:
...
def test_first_blocking_upstream_missing() -> None:
    blocking = get_first_blocking_prerequisite_field(
        form_action="/admin/product-test-releases/create",
        target_field_name="product_test_release_id",
...
def test_validate_identifiers_skips_empty_values() -> None:
    assert validate_product_test_identifier_values({"product_test_release_id": ""}) == []
