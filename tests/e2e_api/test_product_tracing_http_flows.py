...
def test_pt_e2e_007_admin_and_export(seeded_wifi_ap_db: TestClient) -> None:
    ...
    assert 'data-admin-tab-label="Test Round"' in dash.text
    assert f'data-round-id="{RELEASE_ID}"' in dash.text
    assert 'data-admin-tab-label="Runs"' in dash.text
    assert RUN_ID_SEED in dash.text
    assert f'data-run-id="{RUN_ID_SEED}"' in dash.text
...
def test_admin_runs_tab_contains_highlight_links(seeded_wifi_ap_db: TestClient) -> None:
    client = seeded_wifi_ap_db
    ...
    assert f'data-env-id="{ENV_ID}"' in response.text


def test_admin_round_tab_contains_highlight_rows(seeded_wifi_ap_db: TestClient) -> None:
    client = seeded_wifi_ap_db
    response = client.get("/admin", cookies=_cookies("master_admin"))
    assert response.status_code == 200
