...
def test_qc_db_truncate_clears_data_when_qc_mode_on(
    ...
    before = client.get("/admin", cookies=_admin_cookies())
    ...
    after = client.get("/admin", cookies=_admin_cookies())
    ...
    with session_local() as database_session:
        count = database_session.scalar(select(func.count()).select_from(ProductTestRound))
        assert count == 0
