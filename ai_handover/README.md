...
6. **파일은 함수/모듈 단위로 작게.** AI 편집 시 불특정 줄에서 잘리는(torn-write) 사고가 잦으므로, 거대 단일 파일을 만들지 말 것. 새 코드는 기능 단위 파일로 분리하고, 기존 거대 파일을 건드릴 땐 가능하면 해당 부분을 별도 모듈로 분리한다. 권장 상한 — py/js ≈ 400줄, 템플릿/css ≈ 500줄 초과 시 분할 후보. 분리 후 반드시 `node --check`/`py_compile` + 앱 부팅/`pytest`로 동작 보존 확인.
