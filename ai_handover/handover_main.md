...
> 이전 핸드오버는 `ai_handover/archive/HANDOVER_20260604.md` 로 보존.
...
- 탭↔테이블↔TASK 매핑, 연결키 체인(C1~C6), 게이트 조건은 **`ai_handover/master_architecture.md` 참조 (정본).**
...
| 항목 | 값 |
|---|---|
| 위치 | **이 `ai_handover/handover_main.md`가 있는 폴더가 프로젝트 루트** (경로 하드코딩 금지) |
...
- **상세 스펙은 별도 파일 `ai_handover/tasks/task10b_strip_test_prefix.md` 참조.**
...
- **상세 스펙은 별도 파일 `ai_handover/tasks/task11_env_merge.md` 참조.**
...
- **상세 스펙은 별도 파일 `ai_handover/tasks/task12_13_target_merge.md` 참조.**
...
- **상세 스펙은 별도 파일 `ai_handover/tasks/task12b_finalize.md` 참조.**
...
- **상세 스펙은 `ai_handover/tasks/task12_13_target_merge.md` 참조.**
...
- **상세 스펙은 별도 파일 `ai_handover/tasks/task14_release_cleanup.md` 참조.**
...
- **상세: `ai_handover/tasks/task15_v2_migration.md`. 설계 정본: `ai_handover/master_architecture.md` 「v2 구조 결정」.**
