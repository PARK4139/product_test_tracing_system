...
function getFoldState(id) {
    try { return JSON.parse(uiStateGetItem(GANTT_FOLD_KEY) || "{}")[id] === true; } catch(e) { return false; }
}
function saveFoldState(id, folded) {
    try { const s = JSON.parse(uiStateGetItem(GANTT_FOLD_KEY) || "{}"); s[id] = folded; uiStateSetItem(GANTT_FOLD_KEY, JSON.stringify(s)); } catch(e) {}
}
